...
class AgentTemplateRepository:
    ...
    def list_all(self, workspace_path: str = '') -> list[dict]:
        ...
        with self._connect() as conn:
            cursor = conn.execute(
                "SELECT * FROM agent_templates WHERE workspace_path = ? ORDER BY name",
                (workspace_path,)
            )
            return [dict(row) for row in cursor.fetchall()]

    def list_by_source(self, source: str, workspace_path: str = '') -> list[dict]:
        """List templates by source (builtin or user_yaml)."""
        with self._connect() as conn:
            cursor = conn.execute(
    ...
    def create(self, name: str, config_json: str, source: str = 'user_yaml',
               workspace_path: str = '', enabled: bool = True) -> dict:
        """Create a new template."""
        now = datetime.now().isoformat()
        with self._connect() as conn:
            conn.execute(
                """INSERT INTO agent_templates
                   (name, workspace_path, source, config_json, enabled, created_at, updated_at)
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                (name, workspace_path, source, config_json, 1 if enabled else 0, now, now)
            )
            conn.commit()
        return self.get_by_name(name, workspace_path)

    def update(self, name: str, config_json: str, workspace_path: str = '', enabled: Optional[bool] = None) -> Optional[dict]:
        """Update a template."""
        now = datetime.now().isoformat()
        with self._connect() as conn:
    ...
    def upsert(self, name: str, config_json: str, source: str = 'user_yaml',
               workspace_path: str = '', enabled: bool = True) -> dict:
        ...
        if existing:
            ...
            return self.create(name, config_json, source, workspace_path, enabled)
